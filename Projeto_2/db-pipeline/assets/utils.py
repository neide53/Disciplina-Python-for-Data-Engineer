...
def read_metadado(meta_path):
    """
    Função para leitura do arquivo de metadados work_metadado_flights.xlsx
    INPUT: Arquivo xlsx  
    OUTPUT: Dicionário de metadados 
    """
...
def keys_check(df, cols_chaves):
    ...
    tmp = df.copy()
    validacao= len(tmp[cols_chaves].drop_duplicates()) == len(tmp)
    if validacao:
        logger.info(f'Todas as colunas chaves ({cols_chaves}) são únicas; {datetime.datetime.now()}')
    else:
        logger.error(f'Existem duplicidades nas colunas chaves ({cols_chaves}); {datetime.datetime.now()}')
    return validacao
...
def padroniza_str(obs):
    """
    Função para converter string para caixa alta (maiusculo)
    INPUT: String 
...
def corrige_hora(hr_str, dct_hora = {1:"000?",2:"00?",3:"0?",4:"?"}):
    """
    Função para corrigir horas
    INPUT: String 
    OUTPUT: String horario formatado 
    """
    if hr_str == "2400":
        return "00:00"
